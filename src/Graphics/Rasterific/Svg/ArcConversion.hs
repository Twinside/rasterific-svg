...
arc p1@(V2 x1 y1) rxOrig ryOrig x_axis_rotation is_large_arc is_sweep p2@(V2 x2 y2)
    | p1 == p2 = mempty
    | nearZero (abs rx) || nearZero (abs ry) = [LineTo OriginAbsolute [p2]]
    | kCheck == 0 = mempty
    | norm kk == 0 = mempty
    | k5Norm == 0 = mempty
    | otherwise = segs
  where
    ...
    radius@(V2 rx ry)
      ...
      where gamma = (x1_ * x1_) / (rx * rx) + (y1_ * y1_) / (ry * ry)
    ...
    kkk@(V2 k3 k4) = ((-p1) - c_) / radius

    theta1 = abs . acos . min 1 . max (-1) $ k1 / norm kk
